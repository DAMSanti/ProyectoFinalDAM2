...
class ImageHandler {
  ...
  Future<void> loadPhotos(int actividadId) async {
    try {
      ...
      print('[ImageHandler] Error loading photos: $e');
    }
  }
  ...
  Future<bool> uploadSelectedImages(int actividadId) async {
    ...
    print('[ImageHandler] Subiendo ${state.selectedImages.length} imágenes...');
    ...
    for (XFile imageFile in state.selectedImages) {
      try {
        ...
        print('[ImageHandler] Imagen subida: ${imageFile.name}');
      } catch (e) {
        print('[ImageHandler] Error subiendo imagen ${imageFile.name}: $e');
        return false;
      }
    }
  ...
  Future<bool> deleteMarkedImages() async {
    ...
    print('[ImageHandler] Eliminando ${state.imagesToDelete.length} imágenes...');
    
    for (int photoId in state.imagesToDelete) {
      try {
        await state.photoService.deletePhoto(photoId);
        print('[ImageHandler] Imagen eliminada: $photoId');
      } catch (e) {
        print('[ImageHandler] Error eliminando foto $photoId: $e');
        return false;
      }
    }
